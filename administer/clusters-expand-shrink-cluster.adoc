...
向集群添加组件时、您需要 `host_vars`为每个新文件节点和块节点(E-Series阵列)创建文件。需要将这些主机的名称以及要创建的新资源添加到清单中。 `group_vars`需要为每个新资源创建相应的文件。link:../custom/architectures-overview.html["使用自定义架构"^]有关详细信息、请参见一节。
