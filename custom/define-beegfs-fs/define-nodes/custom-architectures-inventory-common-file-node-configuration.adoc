...
beegfs_ha_fencing_agents:
  fence_redfish:
    ...
    - pcmk_host_list: <HOSTNAME>
      ...
      <<: *redfish
----
... 有关配置其他隔离代理的详细信息，请参见 link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/configuring_and_managing_high_availability_clusters/assembly_configuring-fencing-configuring-and-managing-high-availability-clusters["RedHat文档"^]。
...
. 使用cURL或所需工具、将NVIDIA网站上部分中列出的OpenSM版本的软件包下载到目录中 link:beegfs-technology-requirements.html["技术要求"^] `<INVENTORY>/packages/` 。例如：
+
[source, bash]
----
