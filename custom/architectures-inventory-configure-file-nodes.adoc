...
单击 link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/host_vars/beegfs_01.yml["此处"^] 以查看表示单个文件节点的完整清单文件示例。
...
NVIDIA ConnectX-Virtual Protocol Interconnect&reg; (VPI) 适配器支持将 InfiniBand 和以太网作为传输层。模式之间的切换不是自动协商的，必须使用  `mstflint` 中包含的  `mstconfig` 工具进行配置，该工具是 link:https://network.nvidia.com/products/adapter-software/firmware-tools/["NVIDIA软件工具(MFT)"^] 的一部分。只需更改一次适配器的模式即可。这可以手动完成，也可以作为使用清单的  `eseries-[ib|ib_iser|ipoib|nvme_ib|nvme_roce|roce]_interfaces:` 部分配置的任何接口的一部分包含在 Ansible 清单中，以便自动进行检查/应用。
