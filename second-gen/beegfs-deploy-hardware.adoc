...
. 使用 link:beegfs-technology-requirements.html#file-node-requirements["1米InfiniBand HDR 200 GB直连铜缆"] 连接块节点和文件节点，使其与下图所示的拓扑相匹配。
+
...
. 使用适用于您的 InfiniBand 存储交换机的link:beegfs-technology-requirements.html#storage-network-switch-requirements["2米InfiniBand缆线"]，将文件节点上的其余 InfiniBand 端口连接到存储网络的 InfiniBand 交换机。
+
