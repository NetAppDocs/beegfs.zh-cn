...
NOTE: 卷大小指定为整个存储池（也称为卷组）的百分比。NetApp 强烈建议您在每个池中保留一些可用容量，以便为 SSD 过度配置留出空间（有关详细信息，请参阅 https://www.netapp.com/pdf.html?item=/media/17009-tr4800.pdf["NetApp EF600阵列简介"^]）。存储池 `beegfs_m1_m2_m5_m6`也将池容量的 1% 分配给管理服务。因此，对于存储池中的元数据卷 `beegfs_m1_m2_m5_m6`，当使用 1.92 TB 或 3.84 TB 驱动器时，将此值设置为 `21.25`；对于 7.65 TB 驱动器，将此值设置为 `22.25`；对于 15.3 TB 驱动器，将此值设置为 `23.75`。
